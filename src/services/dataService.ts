...
//
// La fuente de verdad es una base de datos PostgreSQL real (PGlite, ver db.ts).
// Para no romper la UI ni volver asíncronas las 11 pantallas, se usa una
// estrategia "caché-a-través-de-BD": localStorage actúa como caché síncrona que
// se hidrata desde PostgreSQL al iniciar la app (ver bootstrap.ts), y cada
// escritura se replica a la base de datos en segundo plano. Si la BD no está
// disponible, la app sigue funcionando contra la caché local (degradación segura).
...
// Empuja una escritura a PostgreSQL sin bloquear la UI. Los errores se registran
...
export const SEED_NOTICIAS: Noticia[] = [
...
export const SEED_OPINIONES: Opinion[] = [
